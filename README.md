...
생성하기보다 **탐색만** 필요한 경우가 많다. 그런데 클로드의 토큰 비용은
계속 증가하고, mcp-atlassian처럼 도구가 많은 MCP는 토큰 소모가 상당하다.
쓰기/읽기를 모두 제공하니 어쩔 수 없는 부분이지만, 조회를 월 10~15번만
해도 회사 할당량을 거의 소모해 버리는 상황이 쉽게 발생한다. 탐색은 그보다
훨씬 빈번한데, 탐색을 읽기 전용으로만 처리해 토큰량을 줄일 방법을
모색하다가 이 프로젝트를 시작했다.
