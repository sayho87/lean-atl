...
<p align="center"><strong>최대 10개 도구. 1.8KB 도구 정의. 도구 정의 토큰 최대 98% 절감.</strong></p>
...
도구 10개, 도구 정의 스키마 1.8KB — 도구 정의를 요청당 약 448토큰만 전송합니다.
Jira를 사용하지 않는 환경(URL 미설정)에서는 Jira 도구 4개가 자동 제외되어
도구 6개 / 1.2KB / 약 304토큰 — 도구가 많은 MCP 대비 도구 정의 기준 최대 98% 절감입니다.
도구 정의는 요청당 한 번 전송되며, 세션 청구액은 읽은 내용·출력·프롬프트 캐시의 영향을 함께 받습니다.
...
같은 API·인증 방식을 쓰는 서버 기준으로, 기본 구성 대비 **스키마 97% 감소
(세션당 약 15,900토큰 절약)**. 최소 구성(5개 도구)으로 줄여도 lean-atl의
10개 도구가 더 가볍습니다 (스키마 평균 169B vs 1,458B).

### 토큰 절약 설계 원칙
1. **도구 10개** — 실제 쓰는 핵심만 (검색/조회/댓글/목록)
2. **docstring 한 줄, 파라미터 설명 최소** — 스키마 평균 169B
